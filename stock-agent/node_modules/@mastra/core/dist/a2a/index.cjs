...
function messagesToPrompt(messages, options) {
  ...
  const converted = chunkRQ4MB5TM_cjs.convertMessages(messages).to("AIV5.Model");
  const lines = [];
...
function createResponseMessages(text, memoryInfo = {}) {
  ...
  return new chunkRQ4MB5TM_cjs.MessageList(memoryInfo).add(
    {
      role: "assistant",
      content: text
    },
...
var A2AAgent = class {
  ...
  async #consumeA2AStream({
    ...
    const messageList = new chunkRQ4MB5TM_cjs.MessageList({ threadId, resourceId });
