...
var chunkPKUBPCPR_cjs = require('../chunk-PKUBPCPR.cjs');
...
function messagesToPrompt(messages, options) {
  ...
  const converted = chunkPKUBPCPR_cjs.convertMessages(messages).to("AIV5.Model");
  const lines = [];
...
function createResponseMessages(text, memoryInfo = {}) {
  ...
  return new chunkPKUBPCPR_cjs.MessageList(memoryInfo).add(
    {
      role: "assistant",
      content: text
    },
...
var A2AAgent = class {
  ...
  async getMemory({ requestContext = new chunkCAVARKYS_cjs.RequestContext() } = {}) {
    if (!this.#memory) {
      return void 0;
    }
  ...
  async #consumeA2AStream({
    ...
    const messageList = new chunkPKUBPCPR_cjs.MessageList({ threadId, resourceId });
